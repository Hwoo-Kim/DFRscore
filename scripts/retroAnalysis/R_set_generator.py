...
def R_set_generator(args, use_inchikey=False, canonicalize=False):
    ...
    if use_inchikey == True:
        ...
        log(
            f"  Converting the input smiles into inchi_key by multiprocessing with {args.num_cores} cores..."
        )
        procs = []
        ...
        log(f"  Size of reactant bag is: {len(reactants)}")
        log(
            f"  Generating canonicalized smiles by multiprocessing with {args.num_cores} cores..."
        )
        procs = []
        with Manager() as manager:
